...
@export var zoom_interpolate_offset : Vector2 = Vector2.ONE
...
func _process(_delta : float) -> void:
	if position_interpolate:
		global_position = global_position.lerp(position_interpolate_target + position_interpolate_offset, position_interpolate_speed)
	
	if rotation_interpolate:
		global_rotation = lerpf(global_rotation, rotation_interpolate_target + rotation_interpolate_offset, rotation_interpolate_speed)
	
	if zoom_interpolate:
		zoom = zoom.lerp(zoom_interpolate_target + zoom_interpolate_offset, zoom_interpolate_speed)