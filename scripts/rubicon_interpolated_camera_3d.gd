...
func _process(delta : float) -> void:
	if position_interpolate:
		global_position = global_position.lerp(position_interpolate_target, position_interpolate_speed * delta)
	
	if rotation_interpolate:
		global_rotation = global_rotation.lerp(rotation_interpolate_target, rotation_interpolate_speed * delta)
	
	if fov_interpolate:
		fov = lerpf(fov, fov_interpolate_target, fov_interpolate_speed * delta)